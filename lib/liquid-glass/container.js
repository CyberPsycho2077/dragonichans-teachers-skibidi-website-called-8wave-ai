    ...
    void main() {
        ...
        for(float x = 0.0; x < 1.0; x += 0.05) {
          for(float yOffset = -5.0; yOffset <= 5.0; yOffset += 1.0) {
            float topY = (containerCenter.y - containerSize.y * 0.4) / textureSize.y;
            float midY = containerCenter.y / textureSize.y;
            float bottomY = (containerCenter.y + containerSize.y * 0.4) / textureSize.y;
        ...
        vec3 tintedColor = mix(color.rgb, sampledGradient, u_tintOpacity * 0.7);
